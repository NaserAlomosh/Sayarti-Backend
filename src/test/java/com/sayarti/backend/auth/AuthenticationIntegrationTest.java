...
class AuthenticationIntegrationTest extends AbstractIntegrationTest {
 ...
 void registersAndHashesPassword() throws Exception {
  ...
  assertThat(response.at("/data/user/email").asText())
          .isEqualTo("sara@example.com");

  var user = users.findByEmailIgnoreCaseAndDeletedAtIsNull("sara@example.com")
          .orElseThrow();

  assertThat(user.getPasswordHash())
          .doesNotContain("StrongPass1");

  assertThat(
          encoder.matches("StrongPass1", user.getPasswordHash())
  ).isTrue();
 }
