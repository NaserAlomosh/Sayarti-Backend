...
public class AuthService {
 private final UserRepository users;private final PasswordEncoder encoder;private final JwtService jwt;private final RefreshTokenService refreshTokens;private final GoogleTokenVerifier googleTokens;
 public AuthService(UserRepository users,PasswordEncoder encoder,JwtService jwt,RefreshTokenService refreshTokens,GoogleTokenVerifier googleTokens){this.users=users;this.encoder=encoder;this.jwt=jwt;this.refreshTokens=refreshTokens;this.googleTokens=googleTokens;}
 ...
 @Transactional public AuthResponse login(LoginRequest r){User user=users.findByEmailIgnoreCaseAndDeletedAtIsNull(normalize(r.email())).orElseThrow(this::invalidCredentials);if(user.getAuthProvider()!=AuthProvider.LOCAL||!encoder.matches(r.password(),user.getPasswordHash()))throw invalidCredentials();return tokens(user);}
 @Transactional public AuthResponse google(GoogleLoginRequest request){
  ...
  User subjectUser=users.findByGoogleSubjectAndDeletedAtIsNull(identity.subject()).orElse(null);
  if(subjectUser!=null){if(!subjectUser.getEmail().equalsIgnoreCase(identity.email()))throw googleFailure();return tokens(subjectUser);}
  String email=normalize(identity.email());
  User emailUser=users.findByEmailIgnoreCaseAndDeletedAtIsNull(email).orElse(null);
  if(emailUser!=null){
   if(emailUser.getAuthProvider()==AuthProvider.LOCAL)throw linkingRequired();
   throw googleFailure();
  }
  User user=User.google(name(identity.givenName()),name(identity.familyName()),email,identity.subject());
  try{users.saveAndFlush(user);}catch(DataIntegrityViolationException exception){throw googleFailure();}
  return tokens(user);
 }
 ...
 private String name(String value){return value==null||value.isBlank()?"Google User":value.trim();}
 private ApiException googleFailure(){return new ApiException(ErrorCode.AUTH_GOOGLE_LOGIN_FAILED,HttpStatus.UNAUTHORIZED,"Google authentication failed");}
 private ApiException linkingRequired(){return new ApiException(ErrorCode.AUTH_ACCOUNT_LINKING_REQUIRED,HttpStatus.CONFLICT,"A local account already uses this email; sign in with password before linking Google");}
}
